...
const StatItem = ({ number, label }) => (
  <div className="stat-item">
    <h3 className='akshay1'>{number}</h3>
    <p>{label}</p>
  </div>
);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const customerData = [
        ...
        { id: 4, lat: -33.86882, lng: 151.209296 }, // Sydney, Australia
        { id: 5, lat: 55.755825, lng: 37.617298 }, // Moscow, Russia
      ];

      const visitorData = [
        { id: 1, lat: 48.856613, lng: 2.352222 }, // Paris, France
        { id: 2, lat: 40.416775, lng: -3.70379 }, // Madrid, Spain
      ...
      setLocations({
        customers: customersWithCountry,
  ...
  return (
    <div className="overview">
      ...
      <p className="overview-description">
        Overview of key metrics and information for your corporate events.
      </p>

      <MetricsSection metrics={keyMetrics} onEdit={handleEdit} />
        ...
        >
          ...
          {locations.customers.map((customer) => (
            <Marker
              ...
              icon={customIcon}
            >
              <Popup>{`Customer: ${customer.country}`}</Popup>
            </Marker>
          ))}
          {locations.visitors.map((visitor) => (
            <Marker
              ...
              icon={customIcon}
            >
              <Popup>{`Visitor: ${visitor.country}`}</Popup>
            </Marker>
          ))}
        </MapContainer>
      </div>
